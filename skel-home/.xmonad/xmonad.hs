...
text_editor			= spawn "xed"
web_browser			= spawn "firefox-developer-edition"

-- Rofi Menus
rofi_asroot 		= spawn "~/.xmonad/rofi/bin/asroot"
rofi_launcher 		= spawn "~/.xmonad/rofi/bin/launcher"
-- rofi_cmd     		= spawn "~/.xmonad/rofi/bin/launcher-cmd"
...
myKeys conf@(XConfig {XMonad.modMask = super}) = M.fromList $
    ...
    , ((super,                      xK_t),      spawn "~/.xmonad/bin/xmoterm.sh -f")

	-- launch applications
    , ((super .|. shiftMask, 		xK_Return), file_manager)
    , ((super .|. shiftMask, 		xK_e), 		text_editor)
    , ((super .|. shiftMask, 		xK_w), 		web_browser)

    -- launch rofi menus
    , ((mod1Mask,           		xK_F1), 	rofi_launcher)
    , ((super,           		    xK_slash), 	rofi_launcher)
